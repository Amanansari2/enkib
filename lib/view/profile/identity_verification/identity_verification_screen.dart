...
import 'package:flutter_projects/view/profile/identity_verification/component/identity_verification_skeleton.dart';
...
class _IdentityVerificationScreenState
  ...
  Future<void> _loadSavedIdentityVerificationData() async {
    ...
    if (authProvider.identityVerificationData != null) {
      ...
      print("Loaded Identity Verification Data----->>>>> $data");
      ...
      setState(() {
        ...
        _transcriptUrl = data['transcript'];
        print("Transcript URL in _loadSavedIdentityVerificationData ----->> $_transcriptUrl");
  ...
  void _setCountryNameFromId() {
    if (_selectedCountryId != null) {
      final selectedCountryName = _countryMap[_selectedCountryId];
      if (selectedCountryName != null) {
        _countryController.text = selectedCountryName;
      }
    }
  }

  void _setStateNameFromId() {
    if (_selectedStateId != null) {
      final selectedStateName = _statesMap[_selectedStateId];
      if (selectedStateName != null) {
        _stateController.text = selectedStateName;
      }
    }
  }
  ...
  Future<void> _fetchIdentityVerification() async {
    ...
    if (token != null) {
      try {
        ...
        print("API Response in _fetchIdentityVerification----->> $response");
        ...
        print("Transcript fetched from API --------->>> $transcript");
        setState(() {
  ...
  Future<void> _submitIdentityVerification() async {
    ...
    print("Transcript URL before submission----->>> $_transcriptUrl");

    if (_transcriptUrl == null || _transcriptUrl!.isEmpty) {
      showCustomToast(context, 'Transcript is required.', false);
      return;
    }
    final file = File(_transcriptUrl!);
    if(!file.existsSync()){
      print('Transcript file does not exist --->>> $_transcriptUrl');
      showCustomToast(context, "Selected transcript file is missing.", false);
      return;
    }
    ...
    if (_dobController.text.isNotEmpty){
      ...
      try{
        final parsedDate = DateFormat('MMMM-dd-yyyy').parse(_dobController.text);
        formattedDate= DateFormat('yyyy-MM-dd').format(parsedDate);
        print("Formated DOb --->>> $formattedDate");
      } catch(e){
        showCustomToast(context, "Invalid date format. Use 'Month-Day-Year'.", false);
        return;
      }
    } else{
      showCustomToast(context, "Date of Birth is required.", false);
    }
      ...
      child: Scaffold(
        ...
        body: _isLoading
          ...
          decoration: BoxDecoration(
            border:
            Border.all(width: 2.0, color: AppColors.dividerColor),
            ...
            children: [
              Text(
                ...
                style: TextStyle(
                    ...
                    color: AppColors.completeStatusTextColor,
                    fontFamily: AppFontFamily.font),
              ),
              SizedBox(height: 16),
              Row(
                children: [
                  SizedBox(width: 10,),
                  Image(image: AssetImage(AppImages.accepted),
                  height: 15,
                    width: 20,
                  ),
                  SizedBox(width: 10,),
              ...
              Row(
                children: [
                  Icon(Icons.pending_actions,
                      color:AppColors.blueColor, size: 24),
                  SizedBox(width: 8),
                  Text(
  ...
  Future<void> _pickImage() async {
    ...
    try{
      ...
      showCustomToast(context, "Permission permanently denied. Please enable it form App Settings.", false);
      openAppSettings();
      }
        ...
        if(pickedFile != null){
          ...
          if(file.existsSync()){
            setState(() {
              ...
              _transcriptUrl = _selectedIdImage!.path;
              print("Transcript file selected ------->>>> $_transcriptUrl");
            });
            print("Transcript file selected ---->>> $_transcriptUrl");
          } else{
          ...
          print("No file selected in _pickIdImage");
        }
      } else if(permissionStatus.isDenied){
        showCustomToast(context, "Permission denied. Please allow in settings.", false);
      }
  ...
  //         print("Transcript file selected----->> $_transcriptUrl");
  //       });
  //       print("Transcript file exists and set: $_transcriptUrl");
  //     }else {
  //       print("Picked file does not exist: ${pickedFile.path}");
  //     }
  //   }else {
