...
import 'package:flutter_projects/view/profile/identity_verification/identity_verification_screen.dart';
...
import 'package:flutter_projects/view/tutor/saved_tutors.dart';
...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (authProvider.userData != null) {
        String balance = (authProvider.userData?['user']?['balance'] is double
            ? (authProvider.userData?['user']?['balance'] as double).toStringAsFixed(2)
            : double.tryParse(authProvider.userData?['user']?['balance']
            ?.replaceAll(RegExp(r'[^\d.]'), '') ?? '')?.toStringAsFixed(2))
            ?? "\$0.0";
  ...
  Widget build(BuildContext context) {
    ...
    final bool profileVerified =
        authProvider.userData?['user']?['profile_completed'] ?? true;
    ...
    final identityRole = settingsProvider.getSetting('data')?['_lernen']
       ...
       textDirection: Localization.textDirection,
        child: Scaffold(
          ...
          appBar: PreferredSize(
            ...
            child: Container(
              padding: EdgeInsets.only(left: 15.0, top: 50,right: Localization.textDirection == TextDirection.rtl?5.0:0.0),
              child: Row(
                children: [
                  CircleAvatar(
                        ...
                        ListTile(
                          ...
                          onTap: () {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                  builder: (context) => FavoritesTutorsScreen()),
                            );
                          },
                        ),
                      if (role == "student")
                ...
                Padding(
                  ...
                  child: Consumer<AuthProvider>(
                    builder: (context, authProvider, child) {
                      final userData = authProvider.userData;
                      String rawBalance = userData?['user']?['balance']?.toString() ?? "\₹0.0";
                      String cleanedBalance = rawBalance.replaceAll("\₹", "").trim();

                      double balanceValue = double.tryParse(cleanedBalance) ?? 0.0;
                      String displayBalance = "\₹" + balanceValue.toStringAsFixed(2);
