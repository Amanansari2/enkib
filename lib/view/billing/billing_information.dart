...
class _BillingInformationState extends State<BillingInformation> {
  ...
  Future<void> _submitBillingInformation() async {
    ...
    setState(() {
      _firstNameError =
          _isFirstNameValid ? null : "${Localization.translate("first_name_required")}";
      _lastNameError =
          _isLastNameValid ? null : "${Localization.translate("last_name_required")}";
      _companyError = _isCompanyValid ? null : "${Localization.translate("company_required")}";
      _emailError = _isEmailValid ? null : "${Localization.translate("email_address_required")}";
      _phoneError =
          _isPhoneNumberValid ? null :"${Localization.translate("phoneNumber")}";
      _countryError = _isCountryValid ? null : "${Localization.translate("country_required")}";
      _cityError = _isCityValid ? null : "${Localization.translate("city_required")}";
      _zipCodeError = _isZipCodeValid ? null : "${Localization.translate("zipCode_required")}";
      _stateError = _isStateValid ? null : "${Localization.translate("state_required")}";
    });
    ...
    if (token != null) {
      ...
      try {
        ...
        if (response['status'] == 200) {
          showCustomToast(context,
              response['message'] ?? "${Localization.translate("billing_successfully")}", true);
          setState(() {
            _onPressLoading = false;
          });
