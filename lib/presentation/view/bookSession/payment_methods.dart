...
import 'package:flutter_projects/presentation/view/bookSession/skeleton/payment_screen_skeleton.dart';
...
class _PaymentScreenState extends State<PaymentScreen> {
      ...
      if (token != null) {
        ...
        try {
          ...
          if (response['status'] == 200) {
            ...
            final cancelMessage = response['message'] ??
                "${Localization.translate("payment_cancelled")}";
            ...
            if (paymentStatus == 'completed') {
              ...
              showCustomToast(
                  context,
                  response['message'] ??
                      "${Localization.translate("payment_processing")}",
                  true);
            }
  ...
  void _navigateToThankYouPage() {
    showCustomToast(
        context, "${Localization.translate("payment_completed")}", true);
  ...
  double _calculateTotal() {
    return widget.cartData.fold(0.0, (sum, session) {
      String priceString =
          session['price'].toString().replaceAll(RegExp(r'[^0-9.]'), '');
  ...
  Widget build(BuildContext context) {
    ...
    String rawBalance = userData?['user']?['balance']?.toString() ?? "";
    String cleanedBalance = rawBalance.replaceAll(RegExp(r'[\$,]'), "").trim();

    double balanceValue = double.tryParse(cleanedBalance) ?? 0.0;

    String displayBalance = "\$" +
        balanceValue.toStringAsFixed(2).replaceAllMapped(
                ...
                child: Column(
                  ...
                  children: [
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        SizedBox(height: 4),
                                        session['subject_group']?.isNotEmpty ==
                                                true
                                            ? Text(
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text('${Localization.translate("grand_total")}',
                            style: TextStyle(
                              fontSize: FontSize.scale(context, 18),
                              color: AppColors.blackColor,
                              fontFamily: AppFontFamily.mediumFont,
